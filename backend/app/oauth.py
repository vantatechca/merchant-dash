...
Modes:
  /oauth/start              → regular sign-in (token row stored, is_admin stays False)
  /oauth/start?mode=admin   → sign-in + auto-promote to admin on callback
                              (demotes any previous admin; only one is_admin=True row)
"""
...
# In-memory state store. Stores both CSRF token and per-flow flags
# (currently just `promote_admin`). Swap for Redis if you scale to >1 worker.
_state_store: dict[str, dict] = {}

# How long a state token is valid before being treated as stale.
_STATE_TTL = timedelta(minutes=10)


def _gc_state_store() -> None:
    """Drop expired state entries. Cheap; called on each /start."""
    cutoff = datetime.utcnow() - _STATE_TTL
    stale = [k for k, v in _state_store.items() if v.get("created", cutoff) < cutoff]
    for k in stale:
...
@router.get("/start")
async def start(request: Request, mode: Optional[str] = None):
    """
    Step 1: redirect the user to Google's consent screen.

    Query params:
        mode=admin  → on callback, promote the resulting token row to is_admin=True
                       (demotes any previously-admin row).
    """
    if not CLIENT_ID:
        raise HTTPException(500, "GOOGLE_CLIENT_ID not set. Add it to your .env file.")
...
async def callback(
    ...
    # Pull flags BEFORE popping so we don't lose them on error paths
    state_data    = _state_store.pop(state)
    promote_admin = bool(state_data.get("promote_admin"))

    # Reject stale state (defense in depth — _gc_state_store usually catches these)
    if datetime.utcnow() - state_data.get("created", datetime.utcnow()) > _STATE_TTL:
        return _close_popup_html(ok=False, message="State expired. Please try again.")
    ...
    if ur.status_code != 200:
        return _close_popup_html(ok=False, message=f"Userinfo failed: {ur.text[:200]}")
    email = ur.json().get("email")
    if not email:
        return _close_popup_html(ok=False, message="Userinfo returned no email.")

    # Store the refresh token. Promotion (is_admin=True) is applied below if requested.
    async with SessionLocal() as session:
        existing = (await session.execute(
            select(OAuthToken).where(OAuthToken.email == email)
        )).scalar_one_or_none()
        ...
        # Auto-promote to admin if /oauth/start was called with ?mode=admin.
        # Demote any other admin first — at most one is_admin=True row at a time.
        if promote_admin:
            others = (await session.execute(
                select(OAuthToken).where(
                    OAuthToken.is_admin == True,                # noqa: E712
                    OAuthToken.email    != email,
                )
            )).scalars().all()
            for o in others:
                o.is_admin = False
    ...
    return _close_popup_html(
        ok      = True,
        message = f"Connected as {email}{' (admin)' if promote_admin else ''}",
        email   = email,
    )
...
def _close_popup_html(ok: bool, message: str, email: Optional[str] = None) -> HTMLResponse:
    """Returns a tiny HTML page that notifies the parent window and closes itself."""
    color      = "#059669" if ok else "#dc2626"
    email_js   = f'"{email}"' if email else "null"
    ok_js      = "true" if ok else "false"
    # JSON-safe message: escape backslash, then double-quote, then newlines
    safe_msg   = (
        message.replace("\\", "\\\\")
               .replace('"', '\\"')
               .replace("\n", "\\n")
