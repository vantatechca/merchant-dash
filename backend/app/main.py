...
#         # Serve real files in dist root (favicon.ico, vite.svg, robots.txt, etc.)
...
# else:
#     log.warning(f"[STATIC] frontend/dist not found at {FRONTEND_DIST} — skipping SPA mount")
...
@api.get("/accounts/{account_id}/test", response_model=AccountTestOut)
async def test_connection(account_id: str, session: AsyncSession = Depends(get_session)):
    acct = await MerchantAccount.get(session, account_id)
    if not acct:
        raise HTTPException(404, "Account not found")
...
# ─── Delete + unlink helpers ─────────────────────────────────────────────────

def _acct_field(acct, key: str, default=None):
    """MerchantAccount.get may return a dict OR an ORM object — handle both."""
    if isinstance(acct, dict):
        return acct.get(key, default)
    return getattr(acct, key, default)


def _classify_users_delete_error(err_msg: str) -> str:
    """
    Map Google's error message to one of the reason codes our frontend
    DeleteAccountModal renders: 'sole_admin' | 'business_manager' | 'api_error'.
    Google's wording isn't a stable contract, so match on substrings and
    adjust this list when new variants show up in logs.
    """
    m = (err_msg or "").lower()
    if any(s in m for s in (
        "last admin", "sole admin", "cannot remove the last",
        "only admin", "last user", "cannot delete the last",
    )):
        return "sole_admin"
    if "business manager" in m or "managed by" in m:
        return "business_manager"
    return "api_error"


async def _revoke_direct_oauth_token(acct) -> None:
    """
    Revoke the OAuth refresh token for a DIRECT-OAuth account.

    DO NOT call this for auth_type='admin' — admin tokens are shared
    across many GMC accounts, and revoking would kill every other connected
    store managed by that same admin Gmail.

    Best-effort: 200 means revoked, 400 usually means already-revoked —
    both are acceptable end states. Other statuses are logged, not raised.
    """
    creds = _acct_field(acct, "credentials_json") or {}
    if not isinstance(creds, dict) or not creds.get("_oauth"):
        return
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        ...
        if r.status_code not in (200, 400):
            log.warning(
                f"[unlink] oauth revoke returned {r.status_code}: {r.text[:200]}"
            )
...
async def delete_account(
    ...
    """
    Delete and fully unlink a Merchant Center account.

    Order of operations matters:
      1. Remove our identity from GMC People & access  (frees the 100-acct slot)
      2. Revoke OAuth token IF auth_type == 'direct'   (skip for shared admin tokens)
      3. Cascade-delete local DB rows

    Step 1 must happen BEFORE step 2 — once OAuth is revoked, the access
    token is dead and we can't call users.delete.

    Response shape consumed by the DeleteAccountModal frontend:
      { ok, deleted, gmc_user_removed, reason, auth_type }
        - gmc_user_removed=True   → slot freed, modal closes with success
        - gmc_user_removed=False  → modal shows partial-success warning
                                    with `reason` telling the user why
    """
    acct = await MerchantAccount.get(session, account_id)
    if not acct:
        raise HTTPException(404, "Account not found")

    auth_type = _acct_field(acct, "auth_type") or "service_account"

    gmc_user_removed = False
    reason: Optional[str] = None

    # Step 1 — frees the GMC People & access slot (critical for the 100-acct cap).
    # GMCClient is sync (requests-based), so offload to a thread.
    try:
        client = await build_client_for(acct)
        await asyncio.to_thread(client.delete_user_me, account_id)
        gmc_user_removed = True
        log.info(
            f"[unlink {account_id}] GMC user removed (auth_type={auth_type})"
        )
    except GMCError as e:
        reason = _classify_users_delete_error(e.message)
        log.warning(
            f"[unlink {account_id}] users.delete failed: "
            f"status={e.status} code={e.code} reason={reason} "
            f"msg={(e.message or '')[:200]}"
        )
    except Exception as e:
        reason = "api_error"
        log.exception(f"[unlink {account_id}] users.delete unexpected error: {e}")

    # Step 2 — revoke OAuth, but ONLY for direct OAuth (not shared admin token).
    if auth_type == "direct":
        try:
            await _revoke_direct_oauth_token(acct)
            log.info(f"[unlink {account_id}] direct OAuth token revoked")
        except Exception as e:
            log.warning(f"[unlink {account_id}] oauth revoke failed: {e}")
    elif auth_type == "admin":
        log.info(
            f"[unlink {account_id}] auth_type=admin — skipping OAuth revoke "
            "(shared token across many accounts)"
        )

    # Step 3 — cascade-delete local rows.
    # MerchantAccount.remove is expected to clean up child rows (products,
    # notifications) either via cascade FK or manual deletes inside .remove().
    await MerchantAccount.remove(session, account_id)
    # Clear sync throttle entry so a future re-link of the same account_id
    # isn't blocked by stale cooldown state.
    _sync_last_run.pop(account_id, None)
    _sync_locks.pop(account_id, None)

    return {
        "ok": True,
        "deleted": True,
        "gmc_user_removed": gmc_user_removed,
        "reason": reason,
        "auth_type": auth_type,
    }
...
@api.get("/oauth/merchant-accounts/{email}")
async def list_merchant_accounts_for_user(
    email: str, session: AsyncSession = Depends(get_session),
):
    log.info(f"[MC-LIST] Request for email={email}")
...
@api.post("/oauth/connect", status_code=201)
async def connect_via_oauth(
    payload: OAuthConnectIn, session: AsyncSession = Depends(get_session),
):
    token = (await session.execute(
...
@api.get("/accounts/{account_id}/datasources")
async def list_datasources(account_id: str, session: AsyncSession = Depends(get_session)):
    acct = await MerchantAccount.get(session, account_id)
    if not acct:
        raise HTTPException(404, "Account not found")
...
@api.post("/datasources", status_code=201)
async def create_datasource(payload: DataSourceIn, session: AsyncSession = Depends(get_session)):
    acct = await MerchantAccount.get(session, payload.account_id)
    if not acct:
        raise HTTPException(404, "Account not found")
...
@api.post("/products/sync/{account_id}")
async def sync_account(
    account_id: str,
    force: bool = False,
    session: AsyncSession = Depends(get_session),
...
@api.get("/accounts/{account_id}/subscriptions")
async def list_subscriptions(account_id: str, session: AsyncSession = Depends(get_session)):
    acct = await MerchantAccount.get(session, account_id)
    if not acct:
        raise HTTPException(404, "Account not found")
